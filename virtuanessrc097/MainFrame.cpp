...
WND_ON_COMMAND(ID_VIEW_PPU_MEMORY, OnViewCommand)

//Dump
WND_ON_COMMAND(ID_DUMP_DUMPCPUMEMORY, OnDumpCommand)
WND_ON_COMMAND(ID_DUMP_DUMPPPUMEMORY, OnDumpCommand)

//Type
WND_ON_COMMAND(ID_TOOL_TYPECLIPBOARD, OnTypeCommand)
//Copy
WND_ON_COMMAND(ID_TOOL_COPYSCREEN, OnCopyCommand)
...
//	DEBUGOUT( "CMainFrame::OnCreate\n" );
	...
	m_uTimerID = (UINT)::SetTimer( m_hWnd, 0x0001, 1000, NULL );
...
//	::KillTimer( m_hWnd, m_uKeyTimerID );
	...
	m_CPUMemoryView.Destroy();
	m_PPUMemoryView.Destroy();
...
WNDCMD	CMainFrame::OnDumpCommand(WNDCMDPARAM)
{
	if (!Emu.IsRunning() || !Nes)
		return;
	...
	CHAR	szTitle[256];
	CApp::LoadString(IDS_UI_SAVEDUMP, szTitle, sizeof(szTitle));
	ofn.lpstrTitle = szTitle;
	...
	ofn.lpstrFile = szFile;
	ofn.nMaxFile = sizeof(szFile);
	ofn.lpstrFilter = "Dump Files\0*.dump\0All File Types\0*.*\0";
	ofn.nFilterIndex = 1;
	ofn.Flags = OFN_OVERWRITEPROMPT | OFN_HIDEREADONLY | OFN_EXPLORER | OFN_PATHMUSTEXIST;
	ofn.lpstrInitialDir = pathstr.c_str();

	if (::GetSaveFileName(&ofn)) {
		std::string path = ofn.lpstrFile;
		std::string converted(path.length(), ' ');

		transform(path.begin(), path.end(), converted.begin(), ::tolower);

		if (converted.rfind(".dump") != converted.size() - 5) {
			path += ".dump";
		}
		...
		if (file != 0) {
			...
			case	ID_DUMP_DUMPCPUMEMORY:
				for (UINT i = 0; i < 8; i++) {
					fwrite(CPU_MEM_BANK[i], sizeof(BYTE), 8192, file);
				}
				break;
			case	ID_DUMP_DUMPPPUMEMORY:
				for (UINT i = 0; i < 12; i++) {
					fwrite(PPU_MEM_BANK[i], sizeof(BYTE), 1024, file);
				}
				break;
			default:
				break;
...
BOOL CMainFrame::GetDataFromClipBoard(std::string& data)
{
...
WNDCMD	CMainFrame::OnTypeCommand(WNDCMDPARAM)
{
	if (!Emu.IsRunning() || !Nes)
		return;
	...
	case	ID_TOOL_TYPECLIPBOARD:
		if (this->GetDataFromClipBoard(data)) {

			DirectInput.TypeText(data);
		}
		break;
	default:
		break;
...
BOOL CMainFrame::SetDataToClipBoard(PBYTE memory, UINT length)
{
	if (memory != 0 && length > 0) {
		...
		HGLOBAL hMen = GlobalAlloc(GMEM_MOVEABLE, length);
		...
		{
			PBYTE lpStr = (PBYTE)GlobalLock(hMen);

			if (lpStr != 0) 
			{
				memcpy(lpStr, memory, length);
...
WNDCMD	CMainFrame::OnCopyCommand(WNDCMDPARAM)
{
	if (!Emu.IsRunning() || !Nes)
		return;

	BYTE buffer[34 * 30 + 1] = { 0 };

	switch (uID) {
	case	ID_TOOL_COPYSCREEN:
		//32*30 ->34*30
		for (UINT i = 0; i < 30; i ++) {
			memcpy((buffer + i * 34), PPU_MEM_BANK[8] + i * 32, 32);
			buffer[i * 34 + 32] = '\r';
			buffer[i * 34 + 33] = '\n';
		}
		//0x2000,0x3C0
		this->SetDataToClipBoard(buffer, sizeof(buffer)/sizeof(BYTE));
		break;
	default:
		break;
	}
...
WNDCMD	CMainFrame::OnViewCommand( WNDCMDPARAM )
{
	...
	switch( uID ) {
		...
		case	ID_VIEW_MEMORY:
			if (!m_CPUMemoryView.m_hWnd) {
				m_CPUMemoryView.Create(HWND_DESKTOP, FALSE);
			}
			::SetWindowPos(m_CPUMemoryView.m_hWnd, HWND_NOTOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE);
			break;
		case	ID_VIEW_PPU_MEMORY:
			if (!m_PPUMemoryView.m_hWnd) {
				m_PPUMemoryView.Create(HWND_DESKTOP, TRUE);
			}
			::SetWindowPos(m_PPUMemoryView.m_hWnd, HWND_NOTOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE);
			break;
		default:
			break;
	}
