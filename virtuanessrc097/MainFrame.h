...
protected:
	...
	BOOL SetDataToClipBoard(PBYTE memory, UINT length);
	BOOL GetDataFromClipBoard(std::string& data);
	...
	WNDCMD		OnDumpCommand(WNDCMDPARAM);
	WNDCMD		OnTypeCommand(WNDCMDPARAM);
	WNDCMD		OnCopyCommand(WNDCMDPARAM);
...
//	CEmuThread	Emu;
	...
	CMemoryView	m_CPUMemoryView;
	CMemoryView	m_PPUMemoryView;
