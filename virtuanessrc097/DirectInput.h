...
#define DEFAULT_TYPING_INTERVAL 20
...
public:
	BOOL HasTypedKeys();
	BOOL LoadTypedKeys();
	void ConfirmTypedKeys();
	USHORT TranslateChar(CHAR c);
	USHORT ComposeKeys(BYTE key1, BYTE key2);
	void TypeKeys(USHORT Keys);
	void TypeText(std::string text);

	void SetCapsLock(BOOL capsLock);
	BOOL GetCapsLock();
	void SetTypingInterval(long typingInterval);
	long GetTypingInterval();
	BOOL IsLetterKey(BYTE key);
