...
LOGFONT	CMemoryView::m_logFont={
	FONTHEIGHT, FONTWIDTH, 0, 0, 0, FALSE, FALSE, FALSE, SHIFTJIS_CHARSET, OUT_TT_PRECIS,
...
BOOL	CMemoryView::Create( HWND hWndParent, BOOL ShowPPUMemory)
{
	this->ShowPPUMemory = ShowPPUMemory;
	...
	HWND hWnd = ::CreateWindowEx(
			WS_EX_TOOLWINDOW,
			VIRTUANES_WNDCLASS,
			ShowPPUMemory ? "PPUMemoryView" : "CPUMemoryView",
...
//	DEBUGOUT( "CMemoryView::OnCreate\n" );
	...
	INT ScrollFull = this->ShowPPUMemory ? 0x2FF : 0xFFF;
	INT ScrollMax = (ScrollFull - m_DispLines) < 0 ? 0 : (ScrollFull + 1 - m_DispLines);
	...
	switch( Keys ) {
		...
		case	'9':
			if( m_DispLines ) {
				...
				BYTE	data = this->ShowPPUMemory 
					? PPU_MEM_BANK[addr >> 10][addr & 0x3FF] 
					: CPU_MEM_BANK[addr >> 13][addr & 0x1FFF];
...
void	CMemoryView::OnDraw( HDC hDC )
{
	...
	for(i = 0; i < m_DispLines; i++ ) {
		...
		for( INT a = 0; a < 16; a++ ) {
			...
			::wsprintf( szTemp, "%1c", ::isprint(data)?data:'.' );
